...
class IPAdapter:
    def __init__(self, pipe, ipadapter_ckpt_path, image_encoder_path, device="cuda", dtype=torch.float16,
    ...
    def init_proj(self):
        image_proj_model = ImageProjModel(cross_attention_dim=self.cross_attention_dim,
    ...
    def init_proj_plus(self):
        image_proj_model = Resampler(dim=self.cross_attention_dim,
                                     depth=4,
                                     dim_head=64,
                                     heads=self.heads,
    ...
    def set_ip_adapter(self):
        ...
        for name in unet.attn_processors.keys():
            ...
            if cross_attention_dim is None:
                ...
                attn_procs[name] = IPAttnProcessor(hidden_size=hidden_size, cross_attention_dim=cross_attention_dim).to(
                    self.device, dtype=self.dtype)
        # todo: Eviatar that's should be extened since time_neti also self.unet.set_attn_processor(XTIAttenProc())
        unet.set_attn_processor(attn_procs)
        if hasattr(self.pipe, "controlnet"):
            raise NotImplementedError("ControlNet not supported")

            if True:  # isinstance(self.pipe.controlnet, MultiControlNetModel):
