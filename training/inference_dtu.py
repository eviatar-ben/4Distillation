...
import constants
import tqdm
from constants import IMAGENET_STYLE_TEMPLATES_SMALL, IMAGENET_TEMPLATES_SMALL, DTU_MASKS
...
def dtu_get_gt_images(cam_idxs, train_data_dir, dtu_lighting, dtu_preprocess_key):
    """
    `dtu_preprocess_key` determines the dimensions we'll use for resizing.
    """
    fnames_gt = [Path(train_data_dir) / TextualInversionDataset.dtu_cam_and_lighting_to_fname(idx, dtu_lighting) for idx
                 in cam_idxs]
...
def time_generate_camidxs_to_preds(train_cfg, cam_idxs, step, num_denoising_steps=30, seeds: List[int] = [0, 1],
                                   eval_placeholder_object_token=None, torch_dtype=torch.float16,
    ...
    (placeholder_view_tokens, placeholder_view_token_ids, placeholder_object_tokens,
     placeholder_object_token_ids) = split_placeholders(placeholder_tokens, placeholder_token_ids)
    ...
    (lookup_camidx_to_view_token,
     lookup_camidx_to_cam_params) = TextualInversionDataset.dtu_generate_dset_cam_tokens_params()

    ## identify tokens not already in the tokenizer, add them, resize embeddings, get token ids
    placeholder_view_tokens_new = [p for p in list(lookup_camidx_to_view_token.values()) if
                                   p not in placeholder_view_tokens]
    num_added_tokens = pipeline.tokenizer.add_tokens(placeholder_view_tokens_new)
    pipeline.text_encoder.resize_token_embeddings(len(pipeline.tokenizer))
    placeholder_view_token_ids_new = [pipeline.tokenizer.convert_tokens_to_ids(t) for t in placeholder_view_tokens_new]
    ...
    if train_cfg.learnable_mode != 0:
        _, mapper_view = CheckpointHandler.load_mapper(
            ...
            embedding_type='view',
            placeholder_time_tokens=placeholder_view_tokens,
            placeholder_time_token_ids=placeholder_view_token_ids, )
    ...
    if train_cfg.learnable_mode in (2, 3, 4, 5) or is_pretrained_object:
        ...
        _, mapper_object_lookup = CheckpointHandler.load_mapper(fname, embedding_type='object',
                                                                placeholder_object_tokens=placeholder_object_tokens,
                                                                placeholder_object_token_ids=placeholder_object_token_ids)

        # if constants.PRETRAINED_MODE0_OBJ:
        #     from models.net_clip_text_embedding import load_pretrained_mapper_object
        #     mapper_object_lookup, _ = load_pretrained_mapper_object()
    ...
    prompt_manager = PromptManager(
        ...
        placeholder_time_token_ids=placeholder_view_token_ids_all,
        placeholder_object_token_ids=placeholder_object_token_ids,
        torch_dtype=torch_dtype)
...
def load_stable_diffusion_model(pretrained_model_name_or_path: str, learned_embeds_path: Path,
                                mapper: Optional[NeTIMapper] = None, num_denoising_steps: int = 50,
...
def split_placeholders(placeholder_tokens, placeholder_token_ids):
    ...
    idxs_view = [i for i, s in enumerate(placeholder_tokens) if s[:5] == "<view"]
    ...
    idxs_object = [i for i, s in enumerate(placeholder_tokens) if s[:5] != "<view"]
    placeholder_object_tokens = [placeholder_tokens[i] for i in idxs_object]
    placeholder_object_token_ids = [placeholder_token_ids[i] for i in idxs_object]
...
def run_inference(prompt: str, pipeline: StableDiffusionPipeline, prompt_manager: PromptManager,
                  seeds: List[int], output_path: Optional[Path] = None, height: int = None,
                  width: int = None, num_images_per_prompt: int = 1, num_denoising_steps=50,
                  guidance_scale=7.5, truncation_idx: Optional[int] = None) -> List[Image.Image]:
    with torch.autocast("cuda"):
        with torch.no_grad():
            prompt_embeds = prompt_manager.embed_prompt(prompt, num_images_per_prompt=num_images_per_prompt,
                                                        truncation_idx=truncation_idx)
    joined_images = []
    for seed in seeds:
        generator = torch.Generator(device='cuda').manual_seed(seed)
        images = sd_pipeline_call(pipeline, num_inference_steps=num_denoising_steps,
...
def get_result_metrics_and_grids(cam_idxs, cam_idxs_train, imgs_pred_all_seeds, imgs_gt,
                                 masks, imgs_gt_plot, seeds, do_lpips=False, title_prefix=""):
    """ 
