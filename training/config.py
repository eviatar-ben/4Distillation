...
class DataConfig:
    ...
    super_category_time_token: Optional[str] = "time"
...
class ModelConfig:
    ...
    # IPAdapter:
    image_encoder_path: Path = "InvokeAI/ip_adapter_sd_image_encoder"
    ip_adapter_path: Path = "ip_adapter_models/ip-adapter-plus_sd15.bin"
    num_image_tokens = 16  # Number of tokens to query from the CLIP image encoding. - when adding video clip this should be 1 todo: IPAdapter if num_image_tokens is not 16 the Resampler won't be load with the pretrained parameters
    ip_hidden_states_scale: float = 0.5
    learn_with_content_mapper = False
    ...
    use_positional_encoding_time: int = 1
    ...
    pe_sigmas_view: Dict[str, float] = field(
        default_factory=lambda: {'sigma_time': 1.0})  # Number of time anchors for computing our positional encodings
    ...
    # configuration for time neti-mappers. Ignored by object neti-mappers
    arch_time_net: int = 0
    arch_time_mix_streams: int = 0
    arch_time_disable_tl: bool = True
    ...
    bypass_unconstrained_time: bool = False
    # size of alpha hyperparameter for the output bypass
    output_bypass_alpha_time: float = 0.2
    ...
    def __post_init__(self):
        if self.pe_sigmas is not None:
            self.pe_sigmas = PESigmas(
                ...
                sigma_theta=self.pe_sigmas.get('sigma_time', 1.0),
                sigma_time=self.pe_sigmas.get('sigma_time', 1.0),
                sigma_r=self.pe_sigmas.get('sigma_time', 1.0),
...
class EvalConfig:
    ...
    validation_seeds: Optional[List[int]] = field(default_factory=lambda: [0, 1, 2])
...
class RunConfig:
    ...
    def __post_init__(self):
        if self.optim.train_batch_size > 3:
            raise ValueError("batch size should be 3 and so should grad accumulation")
