...
def main(infer_cfg: InferenceConfig):
    ...
    mapper_checkpoint_path = infer_cfg.input_dir / f"mapper-steps-{infer_cfg.iteration}_time.pt"
    ...
    if train_cfg.data.camera_representation != 'dtu-12d':
        raise NotImplementedError("inference_old.py script only implemented for dtu dataset")
    ...
    # lookup_camidx_to_time_token, _ = TextualInversionDataset.dtu_generate_dset_cam_tokens_params()
    lookup_camidx_to_time_token = TextualInversionDataset.generate_dset_time_tokens_params()
    cam_idxs = constants.DTU_SPLIT_IDXS['train'] + constants.DTU_SPLIT_IDXS['test']
    lookup_camidx_to_time_token = {k: v for (k, v) in lookup_camidx_to_time_token.items() if k in cam_idxs}
    placeholder_view_tokens = list(lookup_camidx_to_time_token.values())  # is all 64 view tokens
    # todo: why to set the initial token to 'object and not something more semantically closer?
    ...
    token_embeds, placeholder_token_ids, placeholder_view_token_ids, placeholder_object_token_ids = \
        Coach._add_concept_token_to_tokenizer_static(
    ...
    validator = ValidationHandler(train_cfg, placeholder_view_tokens,
                                  ...
                                  fixed_object_token=train_cfg.data.fixed_object_token_or_path,
    ...
    pipeline, _results = validator.infer(accelerator, tokenizer, text_encoder, unet, vae,
                                         num_images_per_prompt=None, seeds=infer_cfg.seeds,
                                         step=infer_cfg.iteration, prompts=None)
    # put it in dictionary form if not using learning mode 3
    if train_cfg.learnable_mode != 3:
        results = {None: _results}  # todo: that is evis' modification should be revisit
    ...
    torch.save(results,
               infer_cfg.inference_dir / f"results_all_iter_{infer_cfg.iteration}_scans_{eval_placeholder_object_tokens}_seeds_{infer_cfg.seeds}.pt")
