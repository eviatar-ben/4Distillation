...
class NeTIPositionalEncoding(nn.Module):
    ...
    def encode(self, t: Union[int, torch.Tensor], l: Union[int, torch.Tensor], debug=False):
        ...
        x = x.cuda()  # (2,)
        v = torch.cat([torch.sin(self.w.detach() @ x), torch.cos(self.w.detach() @ x)])  # (2048,bs)
        if type(t) == int:
            v_norm = v / v.norm()
        else:
...
class BasicEncoder(nn.Module):
    ...
    def encode(self, timestep: torch.Tensor, unet_layer: torch.Tensor) -> torch.Tensor:
        normalized_input = torch.stack(
            [self.normalized_timesteps[timestep.long()], self.normalized_unet_layers[unet_layer.long()]]).T
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        device = x.device
        freqs = torch.exp(torch.arange(0, self.d_model, 2, device=device) * -(math.log(10000.0) / self.d_model))
        angles = x.unsqueeze(-1) * freqs.unsqueeze(0).to(x.device)
        encoding = torch.cat([torch.sin(angles), torch.cos(angles)], dim=-1)  # (bs,1,d_model)
        return encoding[:, 0]
...
class FourierPositionalEncoding(nn.Module):
    ...
    def __init__(self, sigma_x: float, dim: int = 128, normalize=False, seed=0):
        super().__init__()
...
class FourierPositionalEncodingNDims(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = x.T  # (1,bs) # todo: Eviatar: im not sure if the transpose is right - if the feature numbers is different then 1 this won't be right
        x = x.cuda()
        v = torch.cat([torch.sin(self.w.detach() @ x), torch.cos(self.w.detach() @ x)])  # (dim, bs)

        if self.normalize:  # todo: Eviatar - this should be reviewed
            v = v / v.norm(dim=0)
